...
// ─── OUTFIT DETECTION ────────────────────────────────────────
// Detects clothing from any text (Initial Message, description, persona).
// target = 'char' or 'user'
// Only sets if the outfit layer is still empty (won't override already-tracked items).

const OUTFIT_SETS = [
  { re: /\bschool[\s-]*uniform\b/i,                upper: 'school_uniform',   lower: 'school_uniform' },
  { re: /\bmaid[\s-]*(?:outfit|dress|uniform)\b/i, upper: 'maid_outfit',      lower: 'maid_skirt' },
  { re: /\bkimono\b/i,                              upper: 'kimono',            lower: 'kimono' },
  { re: /\byukata\b/i,                              upper: 'yukata',            lower: 'yukata' },
  { re: /\bswimsuits?\b/i,                          upper: 'swimsuit',          lower: 'swimsuit' },
  { re: /\bbikini\b/i,                              upper: 'bikini_top',        lower: 'bikini_bottom' },
  { re: /\b(?:dress|gown|sundress)\b/i,             upper: 'dress',             lower: 'dress' },
  { re: /\b(?:armor|armour)\b/i,                    upper: 'armor',             lower: 'armor' },
  { re: /\brobe\b/i,                                upper: 'robe',              lower: 'robe' },
  { re: /\btracksuit\b/i,                           upper: 'tracksuit_top',     lower: 'tracksuit_bottom' },
  { re: /\b(?:lingerie|negligee)\b/i,               upper: 'lingerie',          lower: 'lingerie' },
  { re: /\b(?:nightgown|nightdress|pajamas?|pyjamas?)\b/i, upper: 'nightgown', lower: 'nightgown' },
  { re: /\bnurse[\s-]*(?:uniform|outfit)\b/i,      upper: 'nurse_uniform',     lower: 'nurse_skirt' },
  { re: /\b(?:business\s*suit|formal\s*suit)\b/i, upper: 'blazer',            lower: 'suit_pants' },
  { re: /\bcheerleader\b/i,                         upper: 'cheer_top',         lower: 'cheer_skirt' },
  { re: /\bwhite\s*(?:dress|gown|robe)\b/i,        upper: 'white_dress',       lower: 'white_dress' },
];

const OUTFIT_UPPER_LIST = [
  { re: /\b(?:shirt|t-shirt|tshirt|tee\b)\b/i,  item: 'shirt' },
  { re: /\bblouse\b/i,                            item: 'blouse' },
  { re: /\b(?:hoodie|sweatshirt)\b/i,             item: 'hoodie' },
  { re: /\b(?:jacket|coat)\b/i,                   item: 'jacket' },
  { re: /\bblazer\b/i,                            item: 'blazer' },
  { re: /\bcrop[\s-]*top\b/i,                    item: 'crop_top' },
  { re: /\btank[\s-]*top\b/i,                    item: 'tank_top' },
  { re: /\b(?:sweater|cardigan|pullover)\b/i,     item: 'sweater' },
  { re: /\bcorset\b/i,                            item: 'corset' },
  { re: /\bpolo\b/i,                              item: 'polo' },
  { re: /\buniform\b/i,                           item: 'uniform' },
  { re: /\btube[\s-]*top\b/i,                    item: 'tube_top' },
  { re: /\bhalter[\s-]*top\b/i,                  item: 'halter_top' },
  { re: /\boff[\s-]*shoulder\b/i,                item: 'off_shoulder_top' },
];

const OUTFIT_LOWER_LIST = [
  { re: /\b(?:jeans?|denim)\b/i,                  item: 'jeans' },
  { re: /\b(?:pants?|trousers?|slacks)\b/i,       item: 'pants' },
  { re: /\b(?:skirt|miniskirt)\b/i,               item: 'skirt' },
  { re: /\bshorts?\b/i,                            item: 'shorts' },
  { re: /\bleggings?\b/i,                          item: 'leggings' },
  { re: /\b(?:sweatpants?|joggers?)\b/i,          item: 'sweatpants' },
];

const OUTFIT_LEGS_LIST = [
  { re: /\b(?:stockings?|thigh[\s-]*highs?)\b/i, item: 'stockings' },
  { re: /\bsocks?\b/i,                             item: 'socks' },
  { re: /\b(?:tights?|pantyhose)\b/i,             item: 'tights' },
];

/**
 * detectOutfit(mem, text, target)
 * Reads any text and sets outfit for 'char' or 'user'.
 * Only fills empty layers — won't override already-tracked items.
 * Adds 'casual_clothes' fallback if nothing detected at all.
 */
export const detectOutfit = (mem, text, target = 'char') => {
  ...
  // Sets (fill both upper & lower)
  for (const s of OUTFIT_SETS) {
    if (s.re.test(text)) {
      if (!upper.includes(s.upper)) upper.push(s.upper);
      if (!lower.includes(s.lower)) lower.push(s.lower);
    }
  }

  // Individual upper
  for (const u of OUTFIT_UPPER_LIST) {
    if (u.re.test(text) && !upper.includes(u.item)) upper.push(u.item);
  }
  // Individual lower
  for (const l of OUTFIT_LOWER_LIST) {
    if (l.re.test(text) && !lower.includes(l.item)) lower.push(l.item);
  }
  // Legs
  for (const lg of OUTFIT_LEGS_LIST) {
    if (lg.re.test(text) && !legs.includes(lg.item)) legs.push(lg.item);
  }
  ...
  else if (/\b(?:panties?|thong|underwear)\b/i.test(text)) panties = true;

  // Apply — only fill empty layers
  if (upper.length   && outfit.upper.length   === 0) outfit.upper = upper;
  if (lower.length   && outfit.lower.length   === 0) outfit.lower = lower;
  if (legs.length    && outfit.legs.length    === 0) outfit.legs  = legs;
 ...
 * If outfit is STILL empty after all detection attempts,
 * set a generic fallback so AI doesn't think char is naked.
 */
