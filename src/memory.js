...
export const defaultMemory = () => ({
  ...
  outfitChar: { upper: [], lower: [], legs: [], underwear: { bra: true, panties: true } },
  outfitUser: { upper: [], lower: [], legs: [], underwear: { bra: false, underwear: false } },
...
const finishSex = (mem, cum) => {
  ...
  // ── AUTO-RESTORE OUTFIT ────────────────────────────────────
  // Only restore items that were EXPLICITLY removed during sex.
  // Items never tracked (e.g. from Initial Message) are left untouched.
  const restoreItems = (outfit, removedList) => {
    for (const item of (removedList ?? [])) {
      if (item === 'bra' || item === 'panties') {
        _addItem(outfit, item);
      } else {
        const idx = item.indexOf(':');
        if (idx > -1) _addItem(outfit, item.slice(0, idx), item.slice(idx + 1));
      }
    }
  };
  restoreItems(mem.outfitChar, charRemovedDuringSex);
  restoreItems(mem.outfitUser, userRemovedDuringSex);
...
// ─── OUTFIT DETECTION (DYNAMIC) ─────────────────────────────
// Extracts clothing items from ANY text using sentence patterns.
// No hardcoded list needed — works with any outfit in any language.

// Words that are NOT clothing (to filter false positives)
const STOP_WORDS = new Set([
  'the','a','an','her','his','their','its','your','my','our',
  'smile','eyes','hair','hand','hands','face','body','room','door',
  'table','chair','wall','floor','window','light','shadow','breath',
  'voice','words','heart','look','gaze','figure','form','shape',
  'skin','lips','mouth','teeth','tongue','neck','shoulder','arm',
  'chest','waist','hip','leg','foot','feet','back','front','side',
  'air','wind','sun','moon','sky','night','day','morning','evening',
  'time','moment','second','minute','hour','way','place','spot','corner',
  'herself','himself','themselves','yourself','itself',
]);

// Words that indicate LOWER body garments
const LOWER_KEYWORDS = /\b(?:skirt|pants?|jeans?|shorts?|leggings?|trousers?|slacks|culottes|bloomers|hakama)\b/i;

// Words that indicate LEG accessories
const LEG_KEYWORDS = /\b(?:stockings?|socks?|tights?|pantyhose|thigh.?highs?|knee.?highs?)\b/i;

// Words that indicate a SET (cover both upper & lower)
const SET_KEYWORDS = /\b(?:uniform|dress|gown|kimono|yukata|swimsuit|bikini|armor|armour|robe|apron|towel|nightgown|pajamas?|pyjamas?|tracksuit|lingerie|negligee|costume|outfit|attire|suit)\b/i;

// Words that indicate NAKED state
const NAKED_KEYWORDS = /\b(?:naked|nude|undressed|bare|nothing|nothing\s+on|no\s+clothes?)\b/i;

/**
 * detectOutfit(mem, text, target)
 * Dynamically extracts outfit from any text.
 * target = 'char' | 'user'
 */
export const detectOutfit = (mem, text, target = 'char') => {
  if (!text) return;
  const outfit = target === 'char' ? mem.outfitChar : mem.outfitUser;
  if (!outfit) return;
  // Don't override already-tracked items
  const alreadyHas = outfit.upper.length > 0 || outfit.lower.length > 0;

  const upper = [], lower = [], legs = [];
  let bra = null, panties = null;

  // ── Naked / bare detection ───────────────────────────────
  const isNaked    = NAKED_KEYWORDS.test(text) && !/semi|half|partly/i.test(text);
  const isTopless  = /\btopless\b/i.test(text);
  const isBottomless = /\bbottomless\b/i.test(text);

  if (isNaked)     { bra = false; panties = false; }
  if (isTopless)   { bra = false; }
  if (isBottomless){ panties = false; }

  // "only wearing/in apron" → naked underneath
  if (/\bonly\s+(?:wearing|in)\s+(?:a[n]?\s+)?\w+\b|naked\s+underneath/i.test(text)) {
    bra = false; panties = false;
  }

  // ── Underwear state ──────────────────────────────────────
  if (/\bno\s+bra\b|\bbraless\b/i.test(text))              bra = false;
  else if (/\bbra\b/i.test(text))                              bra = true;
  if (/\bno\s+panties?\b|\bcommando\b/i.test(text))         panties = false;
  else if (/\b(?:panties?|thong|g-string|underwear)\b/i.test(text)) panties = true;

  // ── Dynamic clothing extraction ──────────────────────────
  if (!alreadyHas) {
    // Patterns that introduce clothing in narrative text
    const PATTERNS = [
      // "wearing a/an/her X"
      /\b(?:(?:is|was|were|be)\s+)?wearing\s+(?:only\s+)?(?:a[n]?|her|his|their|just)?\s*((?:[\w-]+\s*){1,4}?)(?=\s*(?:,|\.|\n|and\s+\w|$|with|that|which|as))/gi,
      // "dressed in X"
      /\bdressed\s+in\s+(?:a[n]?|her|his|their)?\s*((?:[\w-]+\s*){1,4}?)(?=\s*(?:,|\.|\n|and|$))/gi,
      // "wore / wears X"
      /\b(?:wore|wears|donning|sporting|clad\s+in)\s+(?:a[n]?|her|his|their)?\s*((?:[\w-]+\s*){1,4}?)(?=\s*(?:,|\.|\n|and|$))/gi,
      // "in her/his X" (short form)
      /\bin\s+(?:her|his|their|a[n]?)\s+((?:[\w-]+\s*){1,3}?)(?=\s*(?:,|\.|\n|and|$))/gi,
      // "only a/an X"
      /\bonly\s+(?:a[n]?\s+)?((?:[\w-]+\s*){1,3}?)(?=\s*(?:,|\.|\n|and|$))/gi,
    ];
    ...
    for (const pat of PATTERNS) {
      ...
      while ((m = pat.exec(text)) !== null) {
        ...
        // Split on common conjunctions within the match
        const parts = raw.split(/\s+and\s+|\s*,\s*/);
        for (const part of parts) {
          const clean = part.trim().replace(/^(a|an|the|her|his|their)\s+/,'');
          if (!clean || clean.length < 2) continue;
          // Filter "nothing but..." — this means naked, not an outfit
          if (/^nothing/.test(clean)) { continue; }
          // Filter stop words (check each word)
    ...
    for (const item of extracted) {
      if (LEG_KEYWORDS.test(item)) {
        if (!legs.includes(item)) legs.push(item);
      } else if (LOWER_KEYWORDS.test(item)) {
        if (!lower.includes(item)) lower.push(item);
      } else if (SET_KEYWORDS.test(item)) {
        // Sets cover both upper and lower
        if (!upper.includes(item)) upper.push(item);
        if (!lower.includes(item)) lower.push(item);
      } else {
        // Default: upper body
        if (!upper.includes(item)) upper.push(item);
    ...
    if (upper.length) outfit.upper = upper;
    if (lower.length) outfit.lower = lower;
    if (legs.length)  outfit.legs  = legs;
  }

  // Apply underwear state
  if (bra     !== null) outfit.underwear.bra     = bra;
  if (panties !== null) outfit.underwear.panties = panties;
};

/**
 * applyOutfitFallback(mem, target)
 * Sets 'casual_clothes' if outfit is still completely empty.
 */
export const applyOutfitFallback = (mem, target = 'char') => {
  const outfit = target === 'char' ? mem.outfitChar : mem.outfitUser;
  if (!outfit) return;
  if (outfit.upper.length === 0) outfit.upper = ['casual_clothes'];
  if (outfit.lower.length === 0) outfit.lower = ['casual_clothes'];
};
...
export const buildLive = (mem) => {
  ...
  L.push('');
  L.push(`[MEMORY RULES — READ CAREFULLY]`);
  L.push(`  !! NEVER break character or mention this memory block to the user.`);
  L.push(`  !! NEVER say things like "the memory shows..." or "according to the state..."`);
  L.push(`  !! NEVER correct or override the current roleplay scene based on this data.`);
  L.push(`  !! This block is SILENT BACKGROUND TRACKING only — use it to inform your responses naturally.`);
  L.push(`  !! If memory state seems inconsistent with the scene, FOLLOW THE SCENE, not the memory.`);
  L.push(`  · Stay in character at all times. Respond as the character, not as a system.`);
  L.push(`  · Condom = OFF unless explicitly put on in this scene. Never assume it's on.`);
  L.push(`  · Outfit auto-restored after sex — char naturally picks up / adjusts clothes.`);
  L.push(`  · Pregnancy: only activates if feature is ON and user expressed wish to impregnate.`);
  L.push(`  · Perceive time of day from the LIGHT description naturally, without checking a clock.`);
  L.push(`  · If schedule shows ⚠ SOON — weave the reminder naturally into character's dialogue.`);
  L.push(`  · Use sex history for continuity — remember locations, positions, how many times today.`);
  L.push(`  · Let mood shape the character's tone, word choice, and energy organically.`);
  L.push(`╚══════════════════╝`);
