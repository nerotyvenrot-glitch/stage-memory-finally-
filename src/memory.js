...
// ─── INITIAL OUTFIT DETECTION ────────────────────────────────
// Detects what character is wearing from Initial Message text.
// Only runs once when outfit is empty (first load).
...
const OUTFIT_SETS = [
  ...
  { re: /\b(?:nurse\s*uniform|nurse\s*outfit)\b/i, upper: 'nurse_uniform', lower: 'nurse_skirt' },
  { re: /\b(?:office\s*attire|business\s*suit|formal\s*suit)\b/i, upper: 'blazer', lower: 'suit_pants' },
];
...
export const detectInitialOutfit = (mem, msg) => {
  if (!msg) return;
  // Only auto-detect if outfit is still completely empty
  if (mem.outfitChar.upper.length > 0 || mem.outfitChar.lower.length > 0) return;
  ...
  for (const u of OUTFIT_UPPER_LIST) {
    if (u.re.test(msg) && !upper.includes(u.item)) upper.push(u.item);
  }
  // Individual lower
  for (const l of OUTFIT_LOWER_LIST) {
    if (l.re.test(msg) && !lower.includes(l.item)) lower.push(l.item);
  }
  // Legs
  for (const lg of OUTFIT_LEGS_LIST) {
    if (lg.re.test(msg) && !legs.includes(lg.item)) legs.push(lg.item);
  }

  // Underwear state
  if (/\bno\s+bra\b|\bbraless\b/i.test(msg))      bra = false;
  else if (/\bbra\b/i.test(msg))                      bra = true;

  if (/\bno\s+panties?\b|\bcommando\b/i.test(msg)) panties = false;
  else if (/\b(?:panties?|thong|underwear)\b/i.test(msg)) panties = true;

  // Apply to memory
  if (upper.length)      mem.outfitChar.upper = upper;
  if (lower.length)      mem.outfitChar.lower = lower;
  if (legs.length)       mem.outfitChar.legs  = legs;
  if (bra !== null)      mem.outfitChar.underwear.bra     = bra;
  if (panties !== null)  mem.outfitChar.underwear.panties = panties;
};
